...
```
/cvf:plan "I want to add user authentication"
/cvf:plan "What if we had dark mode?"
/cvf:plan "Design a shopping cart feature"
```

## Behavior

1. **Invoke cvf-planner agent** to structure the idea
2. **Save plan** to `.claude-vibe-flow/active_spec.md`
3. **Create TODO items** for implementation tracking

## Output

After running `/cvf:plan`, you'll get:
...
- Or run `/cvf:check` to see current status

## Examples

### Input
```
/cvf:plan "Add a simple search feature to the blog"
```
