...
### 1.1 관리 전략 (Management)
*   **보일러플레이트 유지**: 최신 프레임워크(Next.js, Vite 등)의 '가장 깨끗한' 초기 상태를 템플릿에 포함합니다.
*   **안내용 에이전트 강화**: 유저가 처음 프로젝트를 생성했을 때, 유저를 기획 단계로 이끄는 'Onboarding Agent'를 전면에 배치합니다.

### 1.2 배포 및 업데이트 (Release)
*   **배포**: GitHub 저장소 설정에서 'Template repository'를 활성화하여 웹 상에서 즉시 복사가 가능하게 합니다.
*   **업데이트**: 템플릿 자체의 업데이트는 기존 유저에게 자동 반영되지 않으므로, 중요한 변화가 있을 때 README에 이를 알립니다.
...
### 2.1 관리 전략 (Management)
*   **Infection Engine**: 유저의 프로젝트가 어떤 언어(JS/TS, Go, Python)인지 감지하고, 그에 맞는 에이전트만 골라서 복사해주는 '선별적 이식' 로직을 관리합니다.
*   **Idempotency**: 여러 번 명령어를 쳐도 기존 코드를 파괴하지 않고 설정만 보강하는 '멱등성'을 유지합니다.

### 2.2 배포 및 업데이트 (Release)
*   **배포**: `npm publish`를 통해 전 세계 어디서든 `npx`로 접근 가능하게 합니다. 
*   **업데이트**: 유저가 `npx`를 실행할 때마다 최신 npm 패키지를 사용하므로, 가장 실시간성이 높은 배포 방식입니다.
...
### 3.1 관리 전략 (Management)
*   **규격 준수**: 클로드 코드의 `plugin.json` 사양을 엄격히 준수하여 공식 런타임과의 충돌을 방지합니다.
*   **버전 핀(Pinning)**: 팀 단위 유저가 안정적으로 사용할 수 있도록 버전 정보를 명시하고 호환성을 관리합니다.

### 3.2 배포 및 업데이트 (Release)
*   **배포**: 별도의 빌드 없이 GitHub URL만으로 `claude plugin add`가 가능하게 합니다. 
*   **업데이트**: 클로드 코드 플러그인 관리자 명령어를 통해 유저가 명시적으로 최신 버전을 수혈받을 수 있습니다.
