...
| **Use ASCII Flow Diagrams** | 프로세스·플로우·계획 설명 시 Unicode 박스 드로잉(┌─┐, └─┘, │, ▼, ▲, ◀, ▶)만 사용하고(+- 금지), 상자 내부에는 직관적 이모지 라벨을 우선 배치하며, 단계·계층이 한눈에 보이도록 정렬·화살표 흐름을 유지하고 모바일에서도 읽히도록 폭을 과도하게 늘리지 말 것 |
